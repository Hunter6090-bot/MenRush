...
| --- | --- | --- | --- |
| P0 Video rooms restore temp identity gate before join | this branch `cursor/restore-room-temp-identity-gate-95e1` | — | Cursor cloud — reverses #199 profile-identity join; gate mandatory again (name required, photo optional); never flash real profile; leave-no-trace + occupancy kept; #97–#99 parked; do not ping Al |
...
| P0 Video rooms enter with profile identity (no temp pic gate) | merged `#199` on main (`7a16e8b`) — **superseded** by restore-gate branch | — | Cursor — Zoul 4 Sep 2026 lock; softens #183 gate; join uses profile name/photo; optional temp disguise; leave-no-trace occupancy kept — Al P0: reverse; gate mandatory again |
...
| Official room temp-identity privacy + leave-no-trace (supersedes #150) | PR `#183` `cursor/room-temp-identity-leave-no-trace-139c` | — | Restored as join gate baseline after #199 reversal — temp name required before join; photo optional; leave-no-trace occupancy kept |

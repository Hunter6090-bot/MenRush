...
| --- | --- | --- | --- |
| P0 chat video never finishes download (black player `--:--`) | this branch `cursor/fix-video-message-download-0055` | — | Cursor cloud — Al via Product: open-thread poll re-signed `?access=` every 2.5s and fingerprint included full URL → `<video src>` restarted forever; strip access from fingerprint; lock play src; GET media-url refresh; loading + tap-to-retry; TTL 6h; keep #239 MP4; media lock; BOA90 soft-refresh: send/receive short video note, open, confirm plays; Product squash-merge; do not ping Al |

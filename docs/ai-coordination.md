...
| --- | --- | --- | --- |
| Veriff decision re-poll (missed-webhook recovery) + `/api/verify/webhook` alias | this branch `cursor/veriff-decision-repoll-c9d0` PR `#200` | — | Cursor cloud — BOA90; GET /decision + applyDecision; `/repoll` + `VERIFF_REPOLL_*`; alias shares HMAC handler before JWT; badge only on approved; rebased onto main+#199; #97–#99 parked; do not merge |

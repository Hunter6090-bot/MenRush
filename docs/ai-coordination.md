...
| --- | --- | --- | --- |
| Nearby empty face: transparent cutout + face zoom (`FadedBrandFace`) | this branch `cursor/faded-brand-face-cutout-zoom-12c5` PR `#221` | — | Cursor cloud — Brand re-signed lock: `/brand/medallion-transparent.png` only; no filled disc; CSS face zoom; fade opacity only; empty/missing only (never wipe `/uploads`); not Studio; Chat audited — no FadedBrandFace there (letter/silhouette); BrandMark unchanged; BOA90 glance after Vercel; do not ping Al |

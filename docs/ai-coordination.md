...
| --- | --- | --- | --- |
| Legal Scene colour (Zoul/Product) — hide Cruising/Car/Toilets | this branch `cursor/legal-scene-colour-dd38` | — | Cursor cloud — after #241+#242 on main; GREEN keep Glory hole; RED hide Cruising/Car/Toilets from Scene chips (More filters + Profile editor via shared `DISCOVERY_FILTER_CATEGORIES`); no DB tag rewrite; no outdoor Hot Spots; #97–#99 parked; Product squash-merge without Al wait; do not ping Al |
| Chat no empty-state flash (port #163 onto main+#225/#226/#231) | this branch `cursor/chat-no-empty-flash-ac6d` PR `#242` | — | Cursor cloud — rebased onto main after Product squash-merged #238–#241; seed 1:1 from inbox preview + session cache; gate icebreakers until history confirmed empty; keep merge/appendUnique/stick-to-latest/load-older; supersedes conflicted draft #163; browser-smoke stays public.smoke only; Product squash-merge; do not ping Al |

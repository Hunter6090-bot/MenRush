...
| --- | --- | --- | --- |
| Email confirm gate + Brand welcome pack | this branch `cursor/email-confirm-welcome-pack-4d97` PR `#213` | — | Cursor cloud — Al Approved 8 Sep + Al lock: first confirm/welcome mails Al-only (`al@menrush.com` / BOA90) until `EMAIL_CONFIRM_MAIL_OPEN=true`; transactional only; owner pass on BOA90 first; Product merges after Al receives both + Approves; migration `051_email_confirm` (048–050 taken by #217); #97–#99 parked; do not ping Al; HOLD MERGE until Al BOA90 inbox |

...
| --- | --- | --- | --- |
| P0 phone-fit ALL surfaces at 1× (post-#191/#203/#226) | this branch `cursor/phone-fit-all-surfaces-cb19` PR `#229` | — | Cursor cloud — rebased onto main+#225+#226+#227+#228+#231; Al lock: every route usable at device width scale≈1; global input≥16px + touch-action:manipulation; Rooms h-full; route overflow clip; e2e all shells; keep map pinch + #226 composer 16px; no user-scalable=no; do not ping Al |
| P0 chat scroll-back bounce (stick-to-latest yank) | this branch `cursor/chat-scroll-back-bounce-1f98` | — | Cursor cloud — Al/Zoul lock: while not near latest edge, never auto-scroll on poll/socket/merge/re-render; load-older prepend height-delta; stick only near tip or own send; keep #225 sort/merge + #226/#228 Messaging wins; #163 empty-flash held; BOA90; do not ping Al |
| P0 Nearby map soft continuous pan/pinch (Android+iPhone, post-#224) | this branch `cursor/nearby-map-soft-gestures-51d6` PR `#227` | — | Cursor cloud — rebased onto main+#228+#225+#226; owner lock: stickiness on Android Chrome AND iPhone (not iOS-only); HTML pins steal canvas; drop #224 panBy; markers pointer-events:none + hit-test; soft dragPan; contingency `docs/map-library-swap-brief.md`; BOA90 both phones; do not ping Al |

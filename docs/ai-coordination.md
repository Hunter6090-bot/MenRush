...
| --- | --- | --- | --- |
| P0 Veriff optional signup + duplicate email 400 fix | this branch `cursor/veriff-optional-signup-8b0c` | — | Cursor cloud — P0 signup: honors `ADULT_ASSURANCE_SIGNUP_REQUIRED=false` end-to-end; signup works without Veriff; hardened "Opening Veriff…" with timeout + Skip / Continue without Veriff; `extractApiErrorMessage` / `registerErrorMessage` prioritizes `response.data.error` over Axios generic 400 `err.message`; duplicate email displays clear sign-in and reset-password guidance; draft PR |
| Show distance in miles on Nearby GRID profile cards | this branch `cursor/nearby-grid-miles-distance-837a` | — | Cursor cloud — Nearby grid distance in miles: NearbyProfileGrid shows clear distance badge in top-right corner of card using `getDistanceLabel(user)` (`formatDistanceFromKm`); UK default miles; quiet face; draft PR |
| P0 Veriff signup selfie-only gate (no ID forcing) | this branch `cursor/fix-veriff-selfie-only-a520` | — | Cursor cloud — P0 signup Veriff gate: startSession() passes `features: ['selfid']` + unique vendorData, omitting document payload; documents Veriff Station requirement to provision dedicated Age Estimation/Liveness key (`VERIFF_AGE_ESTIMATION_API_KEY`) when default key is Station IDV product; startIdSession() optional ID path unchanged; draft PR |
| Show distance on other users' profiles (MenRush) | this branch `cursor/show-profile-distance-bc49` | — | Cursor cloud — profile distance: ProfileView & ProfileDrawer show distance from current user in existing app units/style (`DistancePill` / `formatDistanceFromKm`); own profile omitted; privacy/fuzzing/units locks preserved; quiet face; draft PR |
| P0 Nearby map broken Mapbox tiles fix (canvas CSS sizing) | merged `#268` on main (`da5cfe3`) | — | Cursor cloud — P0 LIVE fix: drop .mapboxgl-canvas width:100%!important / height:100%!important CSS override in Discover.tsx and globals.css; Mapbox GL computes devicePixelRatio-scaled buffer and sets inline pixel style; CSS 100% was stretching/distorting the WebGL viewport causing vertical bands/glitches; Discretion slider, disclaimer ×, locate, pins intact; draft PR; Product merges on CI green |
...
| P0 Matches face — empty cutout + Veriff tick + map photo URL (Al launch) | this branch `cursor/matches-face-verified-photo-fc56` | — | Cursor cloud — Al P0: Matches SilhouetteAvatar→FadedBrandFace medallion-transparent; VerifiedBadge tick-only app-wide; getMatches/getReceivedLikes discoveryPhotoUrl(map, main) so ChubbyBear-class map-only faces match Nearby thumbs; media lock; quiet face; draft; do not merge |
| Map pin Discretion slider — replace duplicate RADIUS −/+ (post-#261) | this branch `cursor/nearby-discretion-slider-7567` | — | Cursor cloud — Al ORDER: KEEP bottom All miles dropdown as only search radius; REPLACE top ProximitySlider with Discretion slider → `profiles.map_pin_fuzz_m` (mig 060); banner tracks fuzz range; #261 chrome wins kept (disclaimer ×, no phone Mapbox ±); quiet face; two-men untouched; draft; do not merge |
| Map Legal disclaimer dismiss X + mobile zoom declutter (post-#259/#260) | this branch `cursor/map-disclaimer-dismiss-x-246c` PR `#261` | — | Cursor cloud — Al soft-pass + LOCK: × on Legal banner (localStorage once-read; quiet face exact); top −/+ = search radius (ProximitySlider; "All"=max mi) — **superseded by Discretion slider branch**; kill phone Mapbox ± zoom (pinch); desktop zoom OK; two-men untouched; draft; do not merge; do not ping Al |
| Soft Brand: Events drop Meet in public (post-#259) | this branch `cursor/events-drop-meet-in-public-c8ae` PR `#260` | — | Cursor cloud — Brand soft-OK: Events GPS gate → Consent first. alone (periods only, no middot); kill Meet in public; quiet face; no outdoor marketing swap; two-men mark untouched; draft; do not merge; Brand soft glance when draft lands; do not ping Al |
| P0 discover map chrome declutter + Legal RED banner lock (Al ops busy/clumsy) | this branch `cursor/discover-map-chrome-declutter-2943` PR `#259` | — | Cursor cloud — Al P0 chrome corners + Legal RED quiet face Brand soft-OK exact: independent venues + outdoor locations; KILL commercial-only while #258 outdoor live; no Meet in public / cruising soft-sell; two-men mark untouched; draft PR; do not merge; do not ping Al |
| P0 logo black square → transparent cutout (Brand + Zoul LOCK) | this branch `cursor/logo-cutout-no-black-square-ed4c` | — | Cursor cloud — Al flagged live menrush.com BrandMark/hero/compose in black square; root cause = JPEG-as-PNG black-plate assets (`menrush-logo*.png`); swap chrome to existing `/brand/medallion-transparent.png`; do not crop black-plate; draft PR; do not merge; do not ping Al |
| React Router v7 upgrade (Al ORDER) — PR `#250` | this branch `cursor/react-router-v7-1ee2` | — | Cursor cloud — Al ORDER: RR 6.30→7 library mode; future flags on v6 then bump; React 19 untouched; rebase onto main+#254+#255+#256; no outdoor Hot Spots mix; Product squash-merges after green smoke; do not self-merge; do not ping Al |
| Outdoor residual-risk override South-first (Al/Zoul hold) | this branch `cursor/hotspots-outdoor-override-south-cd7a` | — | Cursor cloud — Al residual-risk OVERRIDE (Legal RED, not sign-off); reopen public visibility commercial OR active ops-curated outdoor; seed `--allow-reactivate` + `ops-curated-override-2026-09-12` for **121 names only** (do NOT invent ~1100); Batch1 soft-inactive unless in list; geocode JSON artifact; draft PR; do NOT merge; do NOT prod-seed (Al widget: South-first vs wait CSV); Scene chips stay hidden; Studio/FAQ holds; do not ping Al |
| Outdoor Batch 1 OFF public map (Al ORDER) — SUPERSEDED visibility | merged `#256` on main (`68e30cf`) | — | Historical: commercial-only visibility; soft-inactive batch1 rows remain; superseded by residual-risk override branch for active ops-curated outdoor path |
...
| Outdoor Hot Spots Batch 1 (Al Legal override) — SUPERSEDED public-on | merged `#253` on main (`87fbdad`) | — | Cursor cloud — historical seed; **public map OFF** via `cursor/hotspots-outdoor-batch1-off-2989`; mig 057 + JSON remain; do not re-seed/re-activate; do not ping Al |
...
| Legal Scene colour (Zoul/Product) — hide Cruising/Car/Toilets | merged `#243` on main (`de877bc`) | — | Cursor cloud — after #241+#242 on main; GREEN keep Glory hole; RED hide Cruising/Car/Toilets from Scene chips (More filters + Profile editor via shared `DISCOVERY_FILTER_CATEGORIES`); no DB tag rewrite; no outdoor Hot Spots; #97–#99 parked; Product squash-merge without Al wait; do not ping Al |
| Chat no empty-state flash (port #163 onto main+#225/#226/#231) | merged `#242` on main (`f542336`) | — | Cursor cloud — rebased onto main after Product squash-merged #238–#241; seed 1:1 from inbox preview + session cache; gate icebreakers until history confirmed empty; keep merge/appendUnique/stick-to-latest/load-older; supersedes conflicted draft #163; browser-smoke stays public.smoke only; Product squash-merge; do not ping Al |
| Discover floating map chat dock (port of #96) | merged `#240` on main | — | Cursor cloud — mig **056** map_feed; Discover-only dock; Product squash-merged |
| Hot Spots GREEN expand (25 + 5 Legal follow-on, Zoul merge-green) | this branch `cursor/hotspots-green-expand-d4a3` PR `#237` | — | Cursor cloud — Legal GREEN only; mig 053–055 + `commercial-venues.green-expand-2026-09.json`; 30 seeded / 0 deferred; soft AMBER held (Centre Stage MCR, Eden Bar, Blayds Bar); Equator Bar Birmingham GREEN; keep-list intact; hand-verified lat/lng; copy name+type+area only; Brand Cruise density claims held; BOA90 soft-refresh via seed script; do not ping Al |
| Empty-mark refine: map full logo + Profile/Grid crop-down brighten | merged `#236` on main (`39221bb`) | — | Cursor cloud — Al via Zoul: empty/missing only; map pin = whole circular medallion (no face crop); Profile empty same 72/96 size + slight crop-down + brighter; Nearby Grid crop-down + brighter; Messages empty shares profile variant; media lock on /uploads; #97–#99 / EMAIL_CONFIRM / Trim / Discover sort parked; Brand glances brighter mark; Product squash-merge (Al skip owner-gate); do not ping Al |
| Nearby Nearest ↔ Latest sort choice | this branch `cursor/nearby-nearest-latest-sort-db83` | — | Cursor cloud — owner ask: explicit Nearest (distance-first, default) vs Latest (fresh faces / newest); localStorage `menrush_nearby_sort`; Status NEW filter kept; tribe/Chat NEW/avatars/media/#97–#99 untouched; Brand glance: `Nearest` / `Latest`; do not ping Al |
| Incoming call ring Trim hook (Legal GREEN) | this branch `cursor/call-ring-trim-hook-fe20` | — | Cursor cloud — Zoul/owner lock: authentic Trim needs Nokia written licence; RED scrape; GREEN interim: wire `/audio/call-ring.trim` + cleared.json hook; placeholder = existing generic Web Audio oscillator + OS push sound; no illegal MP3; Legal brief `docs/legal-call-ring-trim.md`; Brand only reviews cleared file later; do not ping Al |
| Nearby NEW joiners + visitor fresh-face | this branch `cursor/nearby-new-joiners-1493` PR `#230` | — | Cursor cloud — Brand confirmed/locked: same pill `NEW` + helper `Just joined` for joiners (7d) + visitors (HOME_RADIUS_KM=40, VISITOR_TTL_HOURS=48); Status NEW; Chat inbox NEW parked; mig 052; rebased onto main+#229+#231+#227+#228; product squash-merge when MERGEABLE+CI green; do not ping Al |
| P0 phone-fit ALL surfaces at 1× (post-#191/#203/#226) | merged `#229` on main (`288779c`) | — | Cursor cloud — Al lock: every route usable at device width scale≈1; global input≥16px + touch-action:manipulation; Rooms h-full; route overflow clip; e2e all shells; keep map pinch + #226 composer 16px; no user-scalable=no; do not ping Al |
| P0 chat scroll-back bounce (stick-to-latest yank) | merged `#231` on main (`001f26d`) | — | Cursor cloud — Al/Zoul lock: while not near latest edge, never auto-scroll on poll/socket/merge/re-render; load-older prepend height-delta; stick only near tip or own send; keep #225 sort/merge + #226/#228 Messaging wins; #163 empty-flash held; BOA90; do not ping Al |
| P0 Nearby map soft continuous pan/pinch (Android+iPhone, post-#224) | merged `#227` on main (`af62a3e`) | — | Cursor cloud — rebased onto main+#228+#225+#226; owner lock: stickiness on Android Chrome AND iPhone (not iOS-only); HTML pins steal canvas; drop #224 panBy; markers pointer-events:none + hit-test; soft dragPan; contingency `docs/map-library-swap-brief.md`; BOA90 both phones; do not ping Al |
| P0 Speed — Nearby grid/map + Chat thread jank | merged `#228` on main (`8cd630d`) | — | Cursor cloud — rebased onto main+#225+#226; GPS store 15m gate + stable watch; roster fingerprint; Grid card memo; kill burnTick; ChatThreadScroll; inbox debounce; keep #225 reorder merge (sort/appendUnique/window) intact; Live honesty/Brand face/#224 untouched; BOA90; do not ping Al |
| P0 chat timeline reorder (earlier msgs jump) | merged `#225` on main (`200f52a`) | — | Cursor cloud — rebased onto main+#226; Al/BOA90 via Zoul lock; `mergeConversationRows` LIMIT-slid ids; union+sort+cap; keep #226 composer 16px / touch-action / Conversations h-full; #163 held; BOA90; do not ping Al |
...
| P0 Nearby map mobile pan/pinch (post-#216) | merged `#224` on main (`473dc64`) — superseded by soft-gestures `#227` | — | Cursor cloud — HTML pin gesture forward + shell flex; canvas-owned pass-through is the real soft-continuous fix |
...
| Email confirm gate + Brand welcome pack | this branch `cursor/email-confirm-welcome-pack-4d97` PR `#213` | — | Cursor cloud — Al Approved 8 Sep + Al lock: first confirm/welcome mails Al-only (`al@menrush.com` / BOA90) until `EMAIL_CONFIRM_MAIL_OPEN=true`; transactional only; owner pass on BOA90 first; Product merges after Al receives both + Approves; migration `051_email_confirm` (048–050 taken by #217); #97–#99 parked; do not ping Al; HOLD MERGE until Al BOA90 inbox; **P0 orphan fix**: confirm-token + BOA90 `email_confirmed` writes moved inside register txn before COMMIT; post-commit mail best-effort; rebased onto main+#218 |
| Dependabot npm group #218 (14 bumps) | merged `#218` on main (`ac00b0b`) | — | Cursor cloud — pin react-router/dom to 6.30.x (v8 needs React 19); keep other bumps; do not ping Al |
...
| Nearby grid profile sheet: unclipped avatar + readable hierarchy | merged `#214` on main (`93cd5e5`) | — | Cursor cloud — ProfileDrawer: avatar outside overflow-y scroll; distance top-safe; Graham fixture `/dev/profile-sheet`; #97–#99 parked; do not ping Al |
| Get-the-app footer: Sign up free → /register | merged `#212` on main (`8305f47`) | — | Cursor cloud — Brand signed copy; kill Waitlist→`/`; page-local GetTheApp footer only; Terms/Privacy waitlist wording untouched; #97–#99 parked; do not ping Al |
...
| P0 Video rooms restore identity gate (profile OR temp; photo optional) | this branch `cursor/restore-room-temp-identity-gate-95e1` PR `#201` | — | Cursor cloud — Al clarify 4 Sep: gate with Keep real profile + Use temp name; temp photo optional → letter avatar; never block on missing temp pic; leave-no-trace + occupancy kept; softens #199; #97–#99 parked; do not ping Al |
| Chat attach from My Photos albums + contents | this branch `cursor/chat-attach-my-photos-c443` PR `#198` | — | Cursor cloud — rebased onto main+#199+#200; Attach opens My Photos picker (albums + contents); default Public+View once; Private only if opened+picked; POST `/messages/media/from-album` copies bytes, never mutates album/visibility; media lock; DISCREET_MEDIA_BLUR off; Product squash-merge; do not ping Al |
| Veriff decision re-poll (missed-webhook recovery) + `/api/verify/webhook` alias | merged `#200` on main (`68b2ce4`) | — | Cursor cloud — BOA90; GET /decision + applyDecision; `/repoll` + `VERIFF_REPOLL_*`; alias shares HMAC handler before JWT; badge only on approved; rebased onto main+#199; #97–#99 parked |
| P0 Video rooms enter with profile identity (no temp pic gate) | merged `#199` on main (`7a16e8b`) — **superseded** by gate dual-choice `#201` | — | Cursor — Zoul 4 Sep 2026 lock; softens #183 gate; join uses profile name/photo; optional temp disguise; leave-no-trace occupancy kept — Al P0: restore gate with explicit profile OR temp choice |
...
| Public/auth RandomBackground per pathname (no timer) | this branch `cursor/random-bg-per-page-bcc5` PR `#189` | — | Cursor cloud — Zoul lock: stay put on page; re-roll on nav + refresh; avoid last image; login/register/beta via PublicAuthShell; ComingSoon London rooftop untouched; do not mix media/#97–#99/#188 |
| P0 PWA stay signed in + call ring when backgrounded | this branch `cursor/pwa-session-and-call-ring-429e` | — | Cursor cloud — verify: Login redirected even with session; AppShell wiped LS when store empty; SW suppressed call push when "visible"; banner permanent dismiss hid alerts. Fixes: rehydrate+cookie mirror; /login→/app if session; SW always show call; banner snooze not permanent in PWA. Do not weaken 2FA; #97–#99 parked; iOS closed-PWA push needs 16.4+ installed + permission. |
...
| Official room temp-identity privacy + leave-no-trace (supersedes #150) | PR `#183` `cursor/room-temp-identity-leave-no-trace-139c` | — | Baseline for leave-no-trace; join gate now dual-choice (profile OR temp) per Al clarify — temp photo optional with letter avatar |
...
| Merge `origin/main` into five-features (same-intent → main) | this branch `cursor/five-features-community-discreet-1e25` | — | Cursor cloud — took shipped main for Community API, Discreet, Safety, More filters, Nightlife. Nav conflict dropped (Product: comments only). |
...
| Landing leftover copy kill (Video rooms card; dash→period; no invite-only until open; phone tab not Rooms) | PR `cursor/landing-copy-video-rooms-1273` | — | Cursor cloud — Brand sign-off before merge; hero untouched; do not mix #150/#97–#99/#165 |
...
| Discovery filter choices (hookup tags + 60+/custom age) | this branch `cursor/discovery-filter-choices-ac6d` | — | Cursor cloud — rebased #148 onto main+#237; expand vibe/scene/connection/looking_for/body/tribe + age presets 60+/50–59 + custom; keep NEW status + phone-fit; no dating-coded tags; no PnP-as-seek; supersedes draft #148; do not touch #97–#99; do not ping Al |

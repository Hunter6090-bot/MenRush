  ...
  const runQuery = async (text: string, values: unknown[] = []) => {
    ...
    if (sql.startsWith('UPDATE processed_webhook_events')) {
      ...
      if (row) {
        ...
        else if (sql.includes("status = 'ignored_out_of_order'"))
...
// ---------------------------------------------------------------------------
// extractProviderEventId / extractOccurredAt / sanitize
// ---------------------------------------------------------------------------
...
test('buildSafeWebhookMetadata and stripSensitiveWebhookFields omit secrets and card data', () => {
  const meta = buildSafeWebhookMetadata({
    eventId: 'transactionId:tx-1',
    eventType: 'NewSaleSuccess',
    subscriptionId: 'sub-1',
  });
  assert.deepEqual(meta, {
    event_id: 'transactionId:tx-1',
    event_type: 'NewSaleSuccess',
    category: 'activation',
    processor_subscription_id: 'sub-1',
  });
  assert.equal('webhookSecret' in meta, false);

  const stripped = stripSensitiveWebhookFields({
  ...
  assert.equal(stripped.subscriptionId, 'sub-1');
  assert.equal(stripped.webhookSecret, undefined);
  assert.equal(stripped.digest, undefined);
  assert.equal(stripped.cardNumber, undefined);
});

test('parseWebhook does not treat transactionId as subscriptionId', () => {
  const event = ccbillService.parseWebhook({
    eventType: 'NewSaleSuccess',
    transactionId: 'tx-99',
    subscriptionId: 'sub-42',
    timestamp: '2026-01-01T00:00:00Z',
  });
  assert.equal(event.subscriptionId, 'sub-42');
  assert.equal(event.transactionId, 'tx-99');
  assert.equal(event.eventId, 'transactionId:tx-99');
});

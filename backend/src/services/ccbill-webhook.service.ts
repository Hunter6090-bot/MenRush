...
/** Safe summary stored on subscriptions.metadata — never raw postbacks or card data. */
...
const SENSITIVE_KEY_PATTERN =
  /^(webhooksecret|x-webhooksecret|digest|card|cardnumber|accountnumber|cvv|cvc|pan|password|secret|token)$/i;
 ...
 * event with a changed timestamp. Confirm against current CCBill merchant
 * and webhook documentation before relying on this as the sole
 * idempotency defence in production. See issue #48.
 */
...
export function extractOccurredAt(raw: Record<string, string>): Date | null {
  const candidate = raw['timestamp'] || raw['eventDate'] || raw['transactionDate'];
...
/**
 * Build a PCI-safe metadata object for subscriptions / audit summaries.
 * Never persist raw webhook bodies, shared secrets, or card fields.
 */
export function buildSafeWebhookMetadata(input: {
  eventId: string | null;
  eventType: string;
  subscriptionId: string | null;
}): SafeWebhookMetadata {
  return {
    ...
    category: classifyEventCategory(input.eventType),
    processor_subscription_id: input.subscriptionId,
  };
}

/** Drop sensitive keys if a caller ever needs a filtered view of raw fields. */
export function stripSensitiveWebhookFields(
  raw: Record<string, string>,
...
export function createWebhookEventStore(runQuery: QueryFn) {
  return {
    ...
    async markIgnoredOutOfOrder(id: string): Promise<void> {
      ...
      await runQuery(
        `UPDATE processed_webhook_events
         SET status = 'ignored_out_of_order'
         WHERE id = $1`,
        [id],
      );
    },
    ...
    async markFailed(id: string): Promise<void> {
      if (!id) return;
      await runQuery(
        `UPDATE processed_webhook_events SET status = 'failed' WHERE id = $1`,
        [id],
      );
    },
  };

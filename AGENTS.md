...
- **Email**: Resend is primary (`RESEND_API_KEY` secret + `RESEND_FROM_EMAIL`/`RESEND_REPLY_TO` config); Zoho SMTP is the contact-form/fallback path (`ZOHO_SMTP_PASS` secret + `ZOHO_SMTP_USER`/host/port config). `ZOHO_SMTP_USER` must match the mailbox the app password belongs to. `RESEND_FROM_EMAIL` must be on a **Resend-verified domain** — `menrush.com` is verified; the `send.menrush.com` subdomain is not, so send from `hello@menrush.com`.
